...
void EditorSelection::handle_input_event(Urho3D::StringHash event_type, Urho3D::VariantMap& event_data)
{
    ...
    if (name == StringHash("SelectObject")) {
        ...
        for (int i = 0; i < res.Size(); ++i) {
            ...
            if (smg != nullptr) {
                ...
                if (nd != nullptr) {
                    bbtk.output_view()->writeToScreen(QString::number(cr.subObject_) + ": " + QString(nd->GetName().CString()));
