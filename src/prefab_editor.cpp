...
Prefab_Editor::Prefab_Editor(Context * context, void * window_id)
      ...
      draw_debug_(false)
{
    ...
    SubscribeToEvent(E_SCENEUPDATE, URHO3D_HANDLER(Prefab_Editor, handle_scene_update));
    SubscribeToEvent(E_INPUT_TRIGGER, URHO3D_HANDLER(Prefab_Editor, handle_input_event));
    SubscribeToEvent(E_POSTRENDERUPDATE, URHO3D_HANDLER(Prefab_Editor, handle_post_render_update));

    create_visuals();
}

Prefab_Editor::~Prefab_Editor()
{
    camera_controller_->release();
    context_->RemoveSubsystem<Editor_Camera_Controller>();
}

void Prefab_Editor::handle_post_render_update(StringHash event_type, VariantMap & event_data)
{
    if (draw_debug_)
    {
        if (scene_ != nullptr)
